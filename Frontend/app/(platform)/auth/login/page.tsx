...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    // Google OAuth — placeholder for NextAuth integration
    ...
    <main className="flex min-h-screen items-center justify-center px-6">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="relative w-full max-w-md">
        ...
        <div className="rounded-2xl border border-[var(--framify-border)] bg-[var(--framify-card-bg)] p-8 backdrop-blur">
          <h1 className="text-center text-2xl font-bold text-[var(--framify-text)]">Welcome back</h1>
          <p className="mt-2 text-center text-sm text-[var(--framify-text-dim)]">Log in to access your saved customizations</p>

          <div className="mt-6 grid grid-cols-2 gap-3">
            <button type="button" onClick={handleGoogleLogin} className="flex items-center justify-center gap-2 rounded-xl border border-[var(--framify-border)] bg-[var(--framify-card-bg)] px-4 py-2.5 text-sm font-medium text-[var(--framify-text-muted)] transition-colors hover:border-emerald-400/20">
          ...
          <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
            ...
            <button type="submit" className="w-full rounded-xl bg-emerald-400 py-2.5 text-sm font-semibold text-zinc-900 transition-all hover:bg-emerald-300">Log in</button>
          </form>
